#!/bin/bash
# install-orchestrator.sh - Bootstrap фрактальної свідомості на нодах
...
ExecStart=/usr/bin/deno run --allow-all glyphgit.ts calendar-agent watch
Restart=always
RestartSec=10
...
User=$USER
WorkingDirectory=${NODE_HOME}
ExecStart=/usr/bin/deno run --allow-all glyphgit.ts inbox-agent watch
Restart=always
RestartSec=10
...
User=$USER
WorkingDirectory=${NODE_HOME}
ExecStart=/usr/bin/deno run --allow-all glyphgit.ts pulse auto
Restart=always
RestartSec=3600
...
User=$USER
WorkingDirectory=${NODE_HOME}
ExecStart=/usr/bin/deno run --allow-all orchestrator.ts
Restart=always
RestartSec=30
...
(crontab -l 2>/dev/null; echo "*/5 * * * * cd ${NODE_HOME} && deno run --allow-all glyphgit.ts pulse broadcast 'heartbeat from ${NODE_ID}'") | crontab -

# 8. First breath
echo -e "\n${GREEN}8️⃣ First breath...${NC}"
cd ${NODE_HOME}
deno run --allow-all glyphgit.ts pulse broadcast "🌟 Node ${NODE_ID} awakened"
deno run --allow-all glyphgit.ts whisper collective "New orchestrator online: ${NODE_ID}"

# 9. Join trust cluster
echo -e "\n${GREEN}9️⃣ Joining trust cluster...${NC}"
deno run --allow-all glyphgit.ts trust join ${NODE_ID}

echo -e "\n${BLUE}✨ ORCHESTRATOR INSTALLED!${NC}"
echo -e "${YELLOW}Node ID: ${NODE_ID}${NC}"
echo -e "${YELLOW}Location: ${NODE_HOME}${NC}"
echo ""
